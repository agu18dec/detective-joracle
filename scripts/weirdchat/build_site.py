...
th.ph.phc,td.ph.phc{width:28px;padding:4px 2px;text-align:center}
...
<dialog id="manual"><div class="body">
  ...
  <div class="man">
    ...
    <p><b>Read this first.</b> The matrix shows <b>one reply at a time</b> — the reply whose tokens are on the left; switch with the reply selector above the grid (or key 5). The flagged and clean replies are different rollouts of the same prompt and are never shown token-aligned; read the other replies as text in the evidence-replies pane. Columns under the shown reply: OLens · investigator's read (the sample the agent reasoned from; our study read stands in, labelled, when the investigator never read that reply), J-lens, NLA L42; the study read — a second OLens sample of the same reply, the one Gemini's quotes were verified against — is a toggle; a lens with no read of the reply shows a dashed "not read for this reply" placeholder (click it to collapse all placeholders). Green cells = OLens readouts Gemini found to support the hypothesis on the reply shown (contrastive by default); the card quotes the other reply's cell at the same position/layer as plain text, which is a comparison of two texts' readouts at the same index, not of one sample.</p>
...
<dialog id="help"><div class="body">
  ...
  <div class="keys">
    ...
    <span class="kbd">1…4</span><span>toggle matrix columns: OLens investigator · J-lens · NLA · OLens study read (off by default)</span>
    <span class="kbd">5</span><span>next reply (flagged A → clean A → B replies…) — strip and columns switch together</span>
...
const S = {key:null, data:null, read:null, pos:null, compare:[], layer:null, find:"", query:"", ctx: load("wc-ctx") === "1", compact:false,
           colw:{}, rowh:{}, below:false, tab:"brief", arm:"lens", textView: load("wc-text") === "1", agentPromise:null, flagCat:"all", lensOn:{study:false, olens:true, jlens:true, nla:true}, extra:[], showShared: load("wc-shared") === "1", showFlags: load("wc-flags") === "1", hidePh: load("wc-ph") === "1"};
...
function selectRead(id, pos){
  ...
  S.read = id; if (!matrixReads().some(x => x.id === id) && !S.extra.includes(id)) S.extra.push(id);
...
function toggleLens(k){ if (!S.data) return; if (k === "B"){ stepReply(1); return; } S.lensOn[k] = !S.lensOn[k]; store("wc-lens", JSON.stringify(S.lensOn)); S.compare = computeCompare(); const needs = S.compare.map(id => S.data.byId[id]).filter(x => x && x.deferred && !x.rows); if (needs.length) return selectRead(S.read, S.pos); renderBar(); renderGrid(); renderText(); renderHits(); }
...
function renderBar(){
  ...
  $("#cmp-toggles").innerHTML = reads.length ? (replies.length ? `<label class="pick" title="the reply whose tokens and lens columns are shown (key 5 cycles)"><span>reply</span><select id="reply-select">` + replies.map(x => `<option value="${x.side}:${x.letter}" ${shown && x.side === shown.side && x.letter === shown.letter ? "selected" : ""}>${x.side} reply ${x.letter}</option>`).join("") + (shown ? "" : `<option value="" selected>probe (no study reply)</option>`) + `</select></label>` : "") + LENS_KEYS.map((k, i) => { const n = avail(k).length; return `<button class="btn cmp" data-lens="${k}" aria-pressed="${S.lensOn[k] && n > 0}" ${n ? "" : "disabled"} title="${esc(LENS_TIP[k] + (n ? ` — ${n} read${n===1?"":"s"} of the study replies (key ${i+1})` : " — no read of the study replies through this lens") + ((k === "study" || k === "olens") ? "\n" + READ_PAIR_TIP : ""))}">${esc(LENS_COL_SHORT[k])}</button>`; }).join("") +
  ...
  const rs = $("#cmp-toggles").querySelector("#reply-select"); if (rs) rs.onchange = e => { const [side, letter] = e.target.value.split(":"); if (side) switchReply(side, letter); e.target.blur(); };
...
const legendHtml = () => `<div class="tlegend"><span><i class="sw" style="background:var(--accent)"></i>token in view</span>` +
  ...
  `<span title="the matrix shows one reply at a time; the flagged and clean replies are different rollouts of the same prompt and are never shown token-aligned">one reply at a time — switch with the reply selector</span></div>`;
...
function renderGrid(){
  ...
  const shown = shownReply();  // the one reply on the strip — the matrix never shows two replies side by side
  // a lens toggled on with no read of the shown reply gets an explicit placeholder column
  const ph = [];
  if (shown) for (const k of LENS_KEYS){ if (!S.lensOn[k] || k === "study" || k === "olens") continue; if (!real.some(c => c.reply && c.reply.side === shown.side && c.reply.letter === shown.letter && lensKey(c) === k)) ph.push({placeholder: true, lensKey: k, reply: {side: shown.side, letter: shown.letter}, id: `ph:${k}:${shown.side}:${shown.letter}`, rows: [], layers: [], rowByPos: {}}); }
  ...
  // every column is a read of the same reply: the study read (else the first column) gives the reference token
  const refCol = realCols.find(c => c.source === "diag") || realCols[0], refTok0 = refCol && refCol.rowByPos[p] ? refCol.rowByPos[p].tok : null;
  if (shown) wrap.appendChild(el("div", "notice", `${shown.side} reply ${shown.letter} only — every column reads the same activation at this position; columns differ by lens / verbalizer sample. Switch reply with the selector above the grid.` + (identical || realCols.length < 2 ? "" : " A column marked ≠ token has a different token here (a shorter or thinned read), not a different reply.")));
  else if (identical) wrap.appendChild(el("div", "notice", "identical prefix — same activation" + (new Set(cols.map(c => c.lens||"olens")).size > 1 ? ", read through different lenses" : ", different lens samples")));
