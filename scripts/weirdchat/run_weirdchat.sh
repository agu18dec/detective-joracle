...
# the key travels through tmux's environment table, never on a command line ps can read
# (tmux < 3.2 has no `new-session -e`; set-environment works on every version)
